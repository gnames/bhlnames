...
func TestNew(t *testing.T) {
	...
	gnpPool := make(chan gnparser.GNparser, 1)
	gnpPool <- gnp
	...
	for _, v := range tests {
		...
		inp := input.New(gnpPool, opts...)
		if v.id == "" {
			assert.Equal(len(inp.ID), 36, v.msg)
		} else {
			assert.Equal(inp.ID, v.iID, v.msg)
		}

		assert.Equal(inp.RefYearStart, v.iRefYearStart, v.msg)
		assert.Equal(inp.RefYearEnd, v.iRefYearEnd, v.msg)
		assert.Equal(inp.Volume, v.iRefVolume, v.msg)
		assert.Equal(inp.PageStart, v.iRefPageStart, v.msg)
		assert.Equal(inp.PageEnd, v.iRefPageEnd, v.msg)
		assert.Equal(inp.NameString, v.iName, v.msg)
		assert.Equal(inp.CanonicalSimple, v.iCan, v.msg)
	}
}
